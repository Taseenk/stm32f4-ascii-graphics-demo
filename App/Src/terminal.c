...
/**
 * @fn void TerminalProcessData(void)
...
void TerminalProcessData(void)
{
	...
	for (message_length = 0; message_length < rx_received_length; message_length++) {
		...
		if (message_length > (sizeof(rx_message.message) - 1)) {
			...
			TerminalPrint("WARNING: Received message was too long");
		...
		if (s_uart_rx.buffer[buffer_index] == CARRIAGE_RETURN) {
			...
			TerminalPrint("DEBUG: Received: ");
			TerminalPrintNewLine(rx_message.message);
